...
class AuthGroup extends Model {
    ...
    public function addToGroup($uid,$gid){
        ...
        if( is_numeric($uid) && is_numeric($gid) ){
            if($Access->add(array('group_id'=>$gid,'uid'=>$uid)) !==false){
                //echo $Access->getDbError();
                return true;
            }else{
                $this->error = "不能重复添加";
                return false;
    ...
    static public function memberInGroup($group_id){
        ...
        $list     = Db::name() ->field('m.uid,u.username,m.last_login_time,m.last_login_ip,m.status')
                       ->table($l_table.' m')
    ...
    public function checkId($modelname,$mid,$msg = '以下id不存在:'){
        ...
        $s = Db::name($modelname)->where(array('id'=>array('IN',$ids)))->value('id',true);
        if(count($s)===$count){
            return true;
        }else{
